(() => {
  ...
  const mergeTest = (base, overrides) => {
    ...
    const questions = new Map(
      test.blocks.flatMap((block) => block.questions).map((question) => [question.id, question]),
    );
    ...
    Object.entries(overrides.questions || {}).forEach(([id, patch]) => {
      ...
      if (patch.optionSet && overrides.sharedOptions?.[patch.optionSet]) {
        question.options = overrides.sharedOptions[patch.optionSet];
  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (url.pathname.endsWith('/data/test.json')) {
      const [base, overrides] = await Promise.all([
        readJson(raw, init),
        readJson('./data/test-overrides.json', init),
      ]);
    ...
    if (url.pathname.endsWith('/data/careers.json')) {
      ...
      const [parts, familyData] = await Promise.all([
        Promise.all(manifest.parts.map((part) => readJson(part, init))),
        manifest.families
          ? readJson(manifest.families, init)
          : Promise.resolve({ families: {} }),
      ]);
      const careers = parts.flatMap((part) => part.careers || []);
      if (manifest.count && careers.length !== manifest.count) {
        throw new Error(`El catálogo esperaba ${manifest.count} carreras y cargó ${careers.length}.`);
      }
      return jsonResponse({
